...
A third kind of model output sits beside LLM text and virtual-cell numbers: a **typed decision model**. When a TypeSafe block is configured in `.env`, each round sends the contrast, the registered menu, the deterministic check, the menu topology and the virtual-cell briefing to TypeSafe Jev as one state, and receives calibrated answers to a fixed set of typed questions — yes/no, a choice over exactly the registered action identifiers, and a 1-5 score. Because the model cannot emit prose, it cannot name an unregistered action, assert a mechanism, or describe a result. Confident answers become advisory findings in the repair planner's prompt; every answer is recorded as a `TypedJudgment`, which is `model_prediction` and whose `satisfies_premise`, `eliminates_hypothesis` and `is_measurement` are properties fixed at `False`. `JudgmentLedger` scores those probability forecasts against later measured outcomes with a Brier score and revokes a scope that stops calibrating. With no TypeSafe block the critic is not built and the loop is unchanged; `--decision-critic off` disables it explicitly. See [the typed decision model note](TYPED_DECISION_MODEL.md), which also records that the provider's own documentation was unreachable when this was written and which field names are therefore assumptions.
...
- [Typed decision model: what was integrated, its boundary, and what is unverified](TYPED_DECISION_MODEL.md)
